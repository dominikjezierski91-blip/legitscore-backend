...
type Props = {
  ...
  // false dla zalogowanych userów, którzy już zaakceptowali aktualną wersję
  // Regulaminu przy rejestracji — powtórny checkbox byłby zbędnym krokiem.
  // Zastrzeżenie "to ocena ryzyka, nie certyfikat" zostaje pokazane zawsze,
  // bo dotyczy konkretnej analizy, nie samej akceptacji Regulaminu.
  showCheckbox?: boolean;
};

export function SubmissionDisclaimer({ accepted, onChange, showCheckbox = true }: Props) {
  return (
    <section className="glass-card space-y-3 p-5 md:p-6">
      <div className="space-y-2 text-xs text-muted-foreground">
        {showCheckbox ? (
          ...
          <p>
            Przypominamy: analiza LegitScore to ocena ryzyka, a nie certyfikat
            autentyczności.
          </p>
        )}
        <p className="text-xs">
