...
type Props = {
  caseId: string;
  mode?: string;
  autoOpenCollection?: boolean;
  /** Renderuje jako publiczny przykładowy raport (baner + CTA "Sprawdź koszulkę" zamiast "Wstecz"/"Nowa analiza"). */
  isExample?: boolean;
};

export async function CaseReportView({ caseId, mode, autoOpenCollection, isExample }: Props) {
  if (!API_BASE_URL) {
    return (
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL.replace(/\/$/, "")}/api/cases/${caseId}`,
      { cache: "no-store" }
    );
    if (!res.ok) {
  ...
  if (status !== "DECIDED") {
    if (isExample) {
      // Przykładowy raport zawsze powinien być gotowy — jeśli nie jest, nie przekierowujemy
      // anonimowego zwiedzającego do prywatnego /analyze/status, tylko pokazujemy komunikat.
      return (
        <div className="flex flex-1 items-center justify-center">
          <div className="glass-card max-w-md p-6 text-sm text-amber-200">
            Przykładowy raport jest chwilowo niedostępny.
          </div>
        </div>
      );
    }
    const qs = new URLSearchParams();
    qs.set("case_id", caseId);
  ...
  try {
    const url = apiBase + "/api/cases/" + caseId + "/report-data";
